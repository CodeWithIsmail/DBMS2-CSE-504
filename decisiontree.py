    ...
    def build(self):
        ...
        if self.depth < MAX_TREE_DEPTH and len(training_set) >= MINIMUM_SAMPLE_SIZE and len(set([elem["species"] for elem in training_set])) > 1:
            ...
            if max_gain > 0:
                ...
                training_set_l = [
                    elem for elem in training_set if elem[attribute] < split]
                training_set_r = [
                    elem for elem in training_set if elem[attribute] >= split]
                self.left_child = tree_node(
                    training_set_l, self.attribute_list, self.attribute_values, self.depth + 1)
                self.right_child = tree_node(
                    training_set_r, self.attribute_list, self.attribute_values, self.depth + 1)
...
class ID3_tree:
    ...
    def build(self, training_set, attribute_list, attribute_values):
        self.root = tree_node(
            training_set, attribute_list, attribute_values, 0)
        self.root.build()
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        species_samples = {species: [] for species in set(
            instance["species"] for instance in dataset)}

        for instance in dataset:
            species = instance["species"]
            species_samples[species].append(instance)

        for species, samples in species_samples.items():
            test_set.extend(random.sample(samples, 10))
        test_set_dupl = test_set.copy()
        training_set = [
            i for i in dataset if not i in test_set_dupl or test_set_dupl.remove(i)]
        print('dataset size:', len(dataset))
        print('training set size:', len(training_set))
        print('test set size:', len(test_set))

        attr_list = ["sepal_length", "sepal_width",
    ...
    print("accuracy on test set: " + "{:.2f}".format(avg_accuracy * 100) + "%")
